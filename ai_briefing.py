...
def _build_prompt(upcoming, newly_announced, company_context):
    ...
    lines.append(
        ...
        "최근 뉴스 흐름을 반영해서 어떤 종목을 왜 주목해야 하는지 설명해줘. "
        "이때 위에 제공된 컨센서스 EPS 수치나 뉴스 헤드라인 중 최소 1~2개는 문장 안에 "
        "구체적으로 인용해서, 서술이 막연한 통념이 아니라 실제 제공된 데이터에 근거하도록 해줘. "
        "과장하거나 투자 조언처럼 쓰지 말고 사실과 맥락 위주로 요약해줘."
    )
    return "\n".join(lines)
